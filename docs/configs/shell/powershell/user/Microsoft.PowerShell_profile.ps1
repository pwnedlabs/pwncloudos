...
# /home/pwnedlabs/.config/powershell/Microsoft.PowerShell_profile.ps1
...
function prompt {
    ...
    $hostnameColor = if ($user -eq 'root') { "$ESC[31m" } else { "$blue" }  # Red for root
    $dirColor = "$green"
    $promptSymbolColor = if ($user -eq 'root') { "$ESC[31m" } else { "$promptBlue" }  # Different Red color
